...
export default function MyoPizzaDisplay() {
  ...
  const [cartItem, setCartItem] = useState({
    ...
    _id: "",
    image:
      "https://cdn.pixabay.com/photo/2012/04/01/16/39/chefs-hat-23436__340.png",
    description: "",
    varient: "",
    quantity: 1,
    prices: 0,
          ...
          <Box padding="5px">
            ...
            >
              {bases.map((base, index) => {
                return (
                  <option key={index * Math.random()} value={base.name}>
                    <text
                      onClick={() => {
                        const d = { ...cartItem };
                        if (cartItem?.varient === "small") {
                          d.prices =
                            ...
                            base.prices[0][cartItem?.varient] *
                              cartItem?.quantity;
                        } else if (cartItem?.varient === "medium") {
                          d.prices =
                            300 +
                            base.prices[0][cartItem?.varient] *
                              cartItem?.quantity;
                        } else if (cartItem?.varient === "large") {
                          d.prices =
                            400 +
                            base.prices[0][cartItem?.varient] *
                              cartItem?.quantity;
                    ...
                    >
                      {base.name} | {base.prices[0][cartItem?.varient]}
                      /-
                    </text>
                  </option>
                );
            ...
            >
              {cheese.map(cheese => {
                return <option value={cheese.name}>{cheese.name}</option>;
              })}
            </Select>
